...
**GUI-venster**: kies een **bestand** of **map**, zet de **Gevoeligheid** (streng→los;
meer/minder kandidaten), klik **Analyseer**, bekijk de kandidaten in de tabel, kies een
**export-resolutie** uit de dropdown en klik **Exporteer alles**. Een lopende analyse of
export breek je af met **Stop**. Output gaat naar `sample_finder_out/` naast je invoer.
(Geen beeldscherm/tkinter beschikbaar? Dan valt het netjes terug op de console-flow.)
...
Snelste weg: **`--sensitivity 0-100`** (0 = streng/weinig .. 100 = los/veel) zet
meerdere drempels ineen — exact dezelfde knop als de **GUI-slider**. Fijner afstellen
kan per sleutel; prioriteit is
*`--sensitivity` < config-bestand < losse vlaggen < `--set`*:

```powershell
# 0) snelste: één globale knop (zelfde als de GUI-gevoeligheidsslider)
py -3.12 sample_finder.py analyze "track.mp3" --sensitivity 70
